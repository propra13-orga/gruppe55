...
public class Creature_Ice extends Creature {
	
	/** Das neue Projektil fuer das Feuerelementar. */
	
	private Projectile projectile = new Spell_SnowFlake(0,0,0,0);		// Feuerelementare schiessen Feuerbaelle!
	...
	public Creature_Ice (double spawnX, double spawnY, int h, int angr, int vert){
		super (spawnX, spawnY, h, angr, vert);
		
		state[1].changeTexture(Data_Textures.creature_ice); 	// Bild der Lebendigen Kreatur laden
		state[1].defineOffset(0, 4, 0, 4);
		...
		element=2;								// Eiskreatur
		resistances[1][0]=2;					// Eisresistenz
	}
    ...
    public void action(int pX, int pY){
		...
		if(distanceBetween(pX,pY)<=detectionRange){
			...
			if(shootorder > 3){
				shoot((int)angle+5, projectile);
				shoot((int)angle-5, projectile);
				shoot((int)angle+90, projectile);
				shoot((int)angle-90, projectile);
				shootorder = 0;
			}
